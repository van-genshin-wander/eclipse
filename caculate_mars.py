...
def n_body(t, state):
    ...
    moon_pos = state[6:9]
    moon_vel = state[9:12]
    mars_pos = state[12:15]
    mars_vel = state[15:18]
    ...
    # 地球加速度（主要来自太阳）
    earth_acc = acceleration(earth_pos, GM_SUN) - acceleration(moon_pos - earth_pos, GM_MOON) - acceleration(mars_pos - earth_pos, GM_MARS)
    ...
    moon_acc = moon_acc_sun + moon_acc_earth + acceleration(moon_pos - mars_pos, GM_MARS)
    
    #火星加速度（来自太阳、地球、月亮）
    mars_acc = acceleration(mars_pos, GM_SUN) - acceleration(moon_pos - mars_pos, GM_MOON) - acceleration(earth_pos - mars_pos, GM_EARTH)
    
    return np.concatenate([earth_vel, earth_acc, moon_vel, moon_acc, mars_vel, mars_acc])
